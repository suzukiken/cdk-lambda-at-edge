...
export class CdklambdaatedgeStack extends cdk.Stack {
  constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
    ...
    const bucket = new s3.Bucket(this, 'image-derivery-figment-research', {
      bucketName: 'image-derivery-figment-research'
    })
    ...
    const resizer_function = new lambda.Function(this, 'lambda_function', {
      ...
      code: lambda.Code.fromAsset('lambda', {
        bundling: {
          ...
          command: [
            'bash', '-c', `pip install -r requirements.txt -t /asset-output && rsync -r . /asset-output`,
          ],
        },
      }),
      handler: 'image-resizer.handler',
      functionName: 'cdk-lambda-at-edge-image-resizer',
      timeout: cdk.Duration.seconds(15)
    })
    
    const version = resizer_function.addVersion(sha256File('lambda/image-resizer.py'))
